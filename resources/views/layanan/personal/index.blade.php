    ...
    <div class="mt-5 card">
        ...
        <div class="card-body border border-danger">
            <div class="table-responsive">
                <a href="/layanan-personal/create" class="btn btn-primary mb-1 float-end">Create</a>
                        ...
                        <tr id="{{$ly->id}}">
                            ...
                            <td width="150">
                                <a href="{{$ly->image}}" target="_blank">
                                    <img src="{{asset('storage/layanan/personal/'.$basename)}}" class="img-fluid">
                                </a>
                            </td>
                            
                                @if( $ly->status == '1')
                                <td class="text-center text-success"><span class="badge badge-success"></span><strong>Publish</strong></td>
                                @else
                                <td class="text-center text-danger"><span class="badge badge-success"></span> Waiting</td>
                                @endif
                            <td>{{ $ly->created_by }}</td>
                            <td>{{ $ly->created_at }}</td>
                            <td>{{ $ly->updated_at }}</td>
                            <td class="text-center">
                                <a href="/layanan-personal/{{$ly->id}}/edit" class="btn btn-primary" title="Edit"><span class="fa fa-edit"></span></a>
                                <a href="/layanan-personal/{{$ly->id}}" class="btn btn-info" title="Show detail"><span class="fa fa-eye"></span></a>
                                <a data-id = "{{$ly->id}}" class="btn btn-light border border-danger delete-personal" title="Delete"><span class="fa fa-trash text-danger"></span></a>
                            </td>
                        </tr>
                            @php
                                $no++;
