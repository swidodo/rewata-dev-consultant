...
<section class="mt-5">
    ...
    <div class="mt-5 card">
        ...
        <div class="card-body border border-danger">
            <form action="{{ route('layanan-personal.update', $layanan->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row px-5">
                    <div class="form-group mb-3">
                        <label>Title</label>
                        <textarea name="title" class="form-control" rows="10" id="summertitle" name="editordata">{{ $layanan->title }}</textarea>
                    </div>
                    <div class="form-group mb-3">
                        <label>Deskripsi Pendek</label>
                        <textarea name="short_description" class="form-control" rows="10" id="summerdesk">{{ $layanan->short_description }}</textarea>
                    </div>
                    <div class="form-group mb-3">
                        <label>Body</label>
                        <textarea name="body" class="form-control" rows="10" id="summerbody" name="editordata">{{ $layanan->body }}</textarea>
                    </div>
                    ...
                    <div class="col-md-6">
                        @php
                            $basename = basename($layanan->image);
                        @endphp
                        <img id="img-show" src="{{asset('storage/layanan/personal/'.$basename)}}" style="margin-top: 25px; min-width: 30px; min-height : 30px; max-width : 100%;max-height:150;">
                    </div>
                </div>
            </form>
        </div>
